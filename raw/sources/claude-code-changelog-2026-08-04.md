...
Bug fixes and reliability improvements (без деталей в changelog).

---

Примечание о верификации: первый заход через WebFetch на `code.claude.com/docs/en/changelog` (суммаризирующая модель) ошибочно приписал 2.1.221 лимиты параллелизма субагентов (default 20) и per-session cap (default 200) — это уже известные вики лимиты из v2.1.217/v2.1.212 (см. [[claude-code]]), не новые для 2.1.221. Верифицировано прямым запросом verbatim-текста из `raw.githubusercontent.com/.../CHANGELOG.md` — этих пунктов там нет для 2.1.221. Урок: суммаризирующий WebFetch может путать версии при пересказе changelog; для changelog-снапшотов сверять с verbatim-текстом первоисточника, не доверять первой сводке.
