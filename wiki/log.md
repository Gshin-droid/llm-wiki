...
Два следствия: исправления не ждём (сторож — постоянное решение, а не времянка) и не ищем настройку, меняющую адресата, — её не существует. Записана и непроверенная гипотеза обхода: вынести весь `permissions.allow` в `settings.local.json`, оставив в публичном файле только `deny`, `ask` и хуки.
