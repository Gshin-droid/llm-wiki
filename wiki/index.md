...
- [[claude-code]] — CLI-агент Anthropic, движок этой вики; допроверка 09-14: changelog 2.1.271 — дефолт Dynamic Workflows снижен 15→10 (подтверждено системным промптом сессии), `omitClaudeMd` для субагентов, per-command `allowed_domains`, классификатор auto mode проверяет hand-back субагента до передачи; допроверка 09-13: changelog 2.1.268–2.1.270 — `claude plugin eval`, `/goal` больше не замирает молча, утечки секретов/прав доступа плагинов; допроверка 09-10: changelog 2.1.261–2.1.267 — `/skill-doctor`, третий bypass-фикс узла плагинов/маркетплейса, `maxEffortLevel`; допроверка 09-07: команда `/insights` (анализ собственных сессий — «как», а не «сколько», в отличие от `/usage`)
...
- [[claude-skills]] — переносимые "умения" для Claude; допроверка 09-13: `claude plugin eval` — официальный тест-раннер eval-сьюта плагина со скорингом (Claude Code 2.1.269); допроверка 09-10: `/skill-doctor` — диагностика неиспользуемых скиллов и их стоимости в контексте (Claude Code 2.1.261); допроверка 08-31: Skills API (`/v1/skills`), AWS/Microsoft Foundry, три несинхронизированные поверхности, точные лимиты полей SKILL.md
...
- [[openrouter]] — агрегатор нейросетевых моделей; допроверка 09-15: официальный Agent SDK (`@openrouter/agent`, `callModel()`), официальный MCP-сервер (`mcp.openrouter.ai`), алиасы моделей с тильдой на новейшую версию семейства
...
- [[claude-statusline]] — statusline-скрипт для Claude Code CLI (лимиты, директория, git-ветка); допроверка 09-19: без изменений с 08-13 — версия 1.0.6, последний коммит по-прежнему 2026-04-03, проект неактивен
...
- [[gsd-get-shit-done]] — spec-driven агентский фреймворк полного SDLC, мультихост; допроверка 09-16: версия 1.7.0, 9.5k★, новые команды `/gsd-graphify`/`/gsd-mempalace-*`; допроверка 08-12: проект переехал в GSD Core (`open-gsd/gsd-core`), старый репозиторий архивирован
- [[superpowers]] — TDD-ориентированный набор скиллов с автообнаружением, лёгкая альтернатива GSD; автор Jesse Vincent (obra), 288k+ звёзд, 14 хостов установки (допроверка 09-18)
- [[nikita-efimov]] — автор разбора автоматизаций Claude Desktop
- [[prostodevops]] — DevOps-образовательный канал
- [[claude-managed-agents]] — hosted agent harness Anthropic: agent/environment/session/events, память/мультиагентность/self-hosted sandboxes поверх Claude API; допроверка 09-10: permission policy `auto` (сервер сам решает по каждому вызову, с явным предупреждением «не человеческий чекпоинт»), `ant beta:sessions connect` — интерактивное подключение к сессии; допроверка 08-19…09-04 — production-паттерн (webhooks), memory stores, human-in-the-loop, мультиагентная координация, версионирование, MongoDB Atlas data path и экономика координатора, advisor и потолок расходов сессии, скиллы из репозитория и гео-пиннинг инференса, три applied-примера (Data Analyst Agent, Slack Data Bot, SRE Incident Responder) по официальным cookbook'ам — пункт бэклога закрыт полностью, гайдовая 16/16, applied 3/3; допроверка 09-07: `ant apply` — ресурсы (агенты/environments/skills/memory stores/deployments) как файлы в репозитории + `claude-lock.json`, Terraform-style workflow вместо построчных вызовов SDK
...
- [[commerce-agents]] — официальный открытый эталонный блюпринт торгового агента: один агент/один код, три рантайма (Messages API/Agent SDK/Managed Agents), safety-гейты (fencing, provenance, host approval) в общем исполнителе тула
...
- [[apache-maka]] — обвязка в инкубаторе Apache: состояние агента не хранится, а выводится из дописываемого журнала; вторая независимая реализация инварианта «видно — значит записано», плюс T1/T2 и парковка недоказанного; бюджет каталога скиллов 2% окна / 4–8k токенов
...
- [[planning-with-files]] — открытый (MIT) скилл персистентного файлового планирования (task_plan.md/findings.md/progress.md, hash attestation, gated mode); независимая чужая реализация примитива Agent-Maintained Handoff, 60+ поддерживаемых агентов
- [[mcp-for-beginners]] — открытый (MIT) курикулум Microsoft по MCP, 13 модулей + hands-on лабы на 6 языках; 17.2k★, живой; практическая сторона протокола, которой в вики не было — дизайн тула, security-модель сервера
...
- [[ai-security-by-design]] — как ИИ изменил кибербезопасность и принципы защиты (безопасность самого агента); 09-14: три угрозы авторизации MCP (tool poisoning, token passthrough, confused deputy) — доверие к чужой аутентификации, отдельный класс от доверия к чужому содержимому (ContextCrush)
...
- [[project-documentation-vault-pattern]] — Obsidian-вики как документация кодового проекта, альтернатива персональной вики; допроверка 09-12: `obsidian-mcp-plugin` вырос из беты в опубликованный плагин «Semantic Notes Vault MCP», MCP-сервер встроен в сам плагин — мост через отдельный Local REST API + внешний Node-процесс больше не нужен
...
- [[mcp-model-context-protocol]] — открытый протокол подключения агентов к внешним сервисам: архитектура, крупнейшая ревизия спецификации 2026-07-28 — вышла финалом 28.07 (stateless core, MRTR, MCP Apps, Tasks, CIMD); допроверка 09-11: новой версии спецификации/поста дорожной карты после 08-22 нет, Enterprise-Managed Authorization подтверждена первоисточником напрямую (была только по InfoQ); 09-14: практическая сторона от курикулума Microsoft — дизайн тула, retry для эффектных операций, три угрозы авторизации (tool poisoning, token passthrough, confused deputy)
...
- [[agent-teams]] — команды агентов Claude Code: когда team вместо субагентов, размеры, failure modes, security-модель; 09-17: академическая таксономия MAST (14 отказов в 3 категориях) поверх известных Claude Code-специфичных failure modes
...
- [[verification-three-levels]] — лестница проверок агента (инварианты в коде → замеры на эталонах → человек), правило спуска на уровень ниже, три вопроса «инвариант или эвристика», почему заблокированный правильный ответ хуже пропущенной ошибки, чеклист к своей обвязке; допроверка 09-13: тихий сбой без отчёта (не путать с тихим успехом) на примере фикса `/goal`
...
Сводки рутины новостей — что изменилось в мире ИИ так, что влияет на работу с ним. Правила — `CLAUDE.md`, «Рутина 1 — новости: два яруса». Новые сверху.

- [[novosti-2026-09-19]] — казахстанский закон «Об искусственном интеллекте» (в силе с 18.01.2026, обязательная маркировка ИИ-контента) взят вне окна, в вики отсутствовал; уязвимость Plugin4Shell в Claude Code/Codex/Copilot/Gemini CLI; уязвимость BragJack захватывает встроенных ИИ-ассистентов пяти браузеров, включая Claude in Chrome
- [[novosti-2026-09-16]] — новый приказ ФСТЭК о защите персональных данных (в силе с 01.09.2026) впервые отдельно регулирует их обработку при использовании ИИ, взят вне окна по правилу действующего регулирования; Google выпустила голосовую модель Gemini 3.8 Live заметно дешевле GPT-Live-1 Astra и Grok Voice
- [[novosti-2026-09-15]] — первая сводка после расширения на два яруса: Anthropic раскрыла промышленный сбор данных с Claude семью китайскими лабораториями (Alibaba/Moonshot/DeepSeek/Z.ai/MiniMax) для дистилляции в свои модели; OpenAI открыла публичную бету Agents API (харнес Codex как управляемый сервис); DeepSeek выпустила V4.1 Flash и тихо перевела V4 Pro на неё же по цене Flash
...
- [[planning-with-files-review]] — практический скилл прямо применимый к своей же работе: три файла держат план/находки/прогресс на диске вместо контекста, снижая дрейф в длинных задачах; hash attestation против порчи плана и gated-режим — независимая реализация Agent-Maintained Handoff из [[long-running-agent-harness]]
- [[claude-cookbook-scheduled-repository-reviewer]] — официальный паттерн ровно того класса, которым живёт эта вики (автономный плановый прогон агента): read-only tools обоснованы отсутствием канала эксфильтрации, path confinement в три независимых слоя, непрерывность через родной `resume_session_id` SDK вместо самодельного файла-хендоффа, скрипт перепроверяет собственную память по файлу состояния
- [[claude-commerce-agents-blueprint]] — официальный эталонный блюпринт торгового агента: один агент на трёх рантаймах (Messages API/Agent SDK/Managed Agents) через общий исполнитель тула, safety-гейты (fencing, provenance, host approval) держатся одинаково на всех трёх именно поэтому — обратная сторона паттерна read-only агента из cookbook выше, применённая к write-агенту
- [[mcp-for-beginners-curriculum]] — открытый курикулум Microsoft по MCP: единственный практический материал вики про MCP ([[web3nity-mcp-guide]]) учил подключать чужие серверы, этот учит строить и защищать свои — дизайн тула, retry для эффектных операций, три угрозы авторизации (tool poisoning, token passthrough, confused deputy); применимо напрямую, если пользователь решит собрать свой MCP-сервер
...
- [[claude-code-changelog-snapshot-2026-09-10]] — changelog Claude Code (в. 2.1.261–2.1.267): `/skill-doctor` — первый штатный инструмент диагностики неиспользуемых скиллов и их стоимости в окне контекста; третий фикс подряд на одном узле контейнмента плагинов/маркетплейса за полтора месяца — поверхность закрывается по частям, не разово

### Все источники
- [[mast-multi-agent-failure-taxonomy]] — официальный репозиторий авторов исследования Berkeley (Cemri и соавторы) о причинах падения мультиагентных систем: полная таксономия из 14 отказов в 3 категориях (Task/Role Execution, Communication & Coordination, Verification Deficiencies) текстом из первоисточника; точные цифры прогона (трассы/фреймворки/kappa/доля отказов) не подтверждены — arXiv и датасет заблокированы
- [[claude-code-changelog-snapshot-2026-09-14]] — снапшот changelog Claude Code (в. 2.1.271): дефолт Dynamic Workflows снижен 15→10 агентов (подтверждено системным промптом сессии этой вики), `omitClaudeMd` для субагентов, per-command `allowed_domains` в auto mode, классификатор проверяет `!`-команды скилла и hand-back субагента иначе
- [[mcp-for-beginners-curriculum]] — открытый курикулум Microsoft (13 модулей): дизайн тула (единственная ответственность, схема с ограничениями), retry-политика для эффектных vs read-only операций, три угрозы авторизации MCP (tool poisoning, token passthrough, confused deputy) — закрывает пробел «как строить и защищать MCP-сервер», которого не было ни на [[mcp-model-context-protocol]], ни на [[ai-security-by-design]]
- [[claude-managed-agents-auto-permission-policy]] — официальные release notes 10.09: permission policy `auto` для Managed Agents (сервер решает по каждому вызову, явно не человеческий чекпоинт), `ant beta:sessions connect` — интерактивное подключение терминалом к сессии, ответ на подтверждение вживую
- [[claude-code-changelog-snapshot-2026-09-13]] — снапшот changelog Claude Code (в. 2.1.268–2.1.270): `claude plugin eval` — официальный тест-раннер плагинов, `/goal` больше не замирает молча после сбоя, утечки секретов в ошибках плагинов/MCP-конфигов, права доступа при распаковке архива плагина, регрессия схемы Artifact-инструмента ломала сторонние Anthropic-совместимые эндпоинты три версии
- [[claude-code-changelog-snapshot-2026-09-10]] — снапшот changelog Claude Code (в. 2.1.261–2.1.267): `/skill-doctor` — диагностика неиспользуемых скиллов и их стоимости в контексте, третий фикс подряд на одном узле контейнмента плагинов/маркетплейса (backslash-обход 2.1.265, macOS/Linux-обход 2.1.267), `maxEffortLevel`, плотный кластер фиксов промпт-кэша вокруг MCP-тулов
- [[claude-commerce-agents-blueprint]] — официальный эталонный блюпринт торгового агента Anthropic: пара агентов (шоппинг/мерчант) на трёх рантаймах через общий `commerce-common`, four-vertical демонстрация (retail/travel/telecom/entertainment), safety-гейты в исполнителе тула
- [[claude-cookbook-scheduled-repository-reviewer]] — официальный recipe Claude Agent SDK Cookbook: read-only ревьюер репозитория на cron, three-layer path confinement, непрерывность через `resume_session_id`
- [[planning-with-files-review]] — наводка из инбокса (без ссылки, без единой проверяемой цифры) на реальный открытый скилл персистентного файлового планирования; репозиторий прочитан напрямую, механика оказалась шире одного абзаца поста
- [[ant-apply-managed-agents-docs]] — официальная документация (`ant` CLI 1.30.0, 03.09.2026): `ant apply` — Terraform-style workflow для Managed Agents, ресурсы описываются файлами в репозитории + `claude-lock.json`, жёсткий отказ при ручной правке через Console вместо тихого перезаписывания
...
- [[apache-maka-architecture-docs]] — семь официальных документов Apache Maka: журнал как источник состояния, две границы фиксации вокруг вызова инструмента, четыре объяснения пропавшего результата, честный список того, чего восстановление не обещает
...
- [[claude-code-practices]] — полный свод правил для Claude Code: безопасность кода, автономная работа, контент/цепочка поставок, оркестрация; что вошло в глобальный CLAUDE.md/скиллы и что отсечено; с 19.08 — п. 12: у смонтированного правила записывается, что его снимает, иначе свод умеет только расти; с 11.09 — п. 14: локальная копия репозитория — снимок, отставание от GitHub видно только после fetch, «не проверено» ≠ «ноль»
...
- [[ustanovka-instrumentov-na-windows]] — четыре грабли, все многоразовые (с 11.09 — п. 4: в окне VS Code над всеми проектами сессия Claude Code получает `.venv` соседнего проекта через `claudeCode.usePythonEnvironment`, выключается настройкой папки): манифест пакетного менеджера ссылается на файл поставщика и умирает вместе с уборкой архива (404 на любой версии, перебор бесполезен); PATH не догоняет уже запущенные окна, и «пропало» означает «перезапусти»; Git Bash не считает `.bat` исполняемым, лечится обёрткой без расширения
