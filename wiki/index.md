...
- [[google-ax]] — открытая среда выполнения агентов поверх Kubernetes (Apache 2.0, v0.3.0): контрольные точки состояния, сетевой allowlist как несущий примитив; цена входа — кластер
- [[m3e-canvas]] — макет собирается мышью в браузере, на выходе промпт для агента-кодера (MIT); две оговорки: хранение только в браузере и словарь компонентов, тянущий за собой Jetpack Compose
- [[material-3-expressive]] — дизайн-система Google (вышла с Android 16): 15 компонентов, акцентная типографика, физика движения; полностью реализована в Jetpack Compose, в React Native поддержка ещё собирается
- [[cloudflare-tunnel]] — публичный HTTPS-адрес локальному серверу одной командой, без аккаунта и проброса портов; где границы (только разработка, 200 запросов, без SSE, адрес меняется при перезапуске)
- [[laya]] — открытая модель типизированных решений (0,3–0,5B, Apache 2.0): вероятность, выбор или оценка за один проход и 33 мс; где выигрывает у платного облака и где честно проигрывает
- [[jev-typesafe]] — платный облачный API типизированных решений, точка отсчёта для [[laya]]; сведения из README конкурента, первоисточником не подтверждены
- [[openjev-verdict-2]] — третья модель типизированных решений (149,6M, Apache 2.0), заявляет превосходство над [[laya]] и [[jev-typesafe]] на собственном тестовом наборе — авторское сравнение, не независимое
- [[nvidia-pair]] — маршрутизатор локального вывода NVIDIA (Apache 2.0, бета): раздаёт запросы к Ollama/LM Studio между машинами одной сети; маршрутизатор, а не кластер — видеопамять не складывается
- [[sol-pi]] — расширение NVlabs к харнесу Pi: −45…49% токенов агента без потери качества, четыре механизма найдены автоматическим перебором, а не спроектированы; не мод к Claude Code, вопреки пересказам
- [[pi-coding-agent]] — минимальный открытый терминальный харнес (Earendil Works, бывш. badlogic/pi-mono): маленькое ядро + расширения на TypeScript, четыре режима запуска; единственная платформа, где работает [[sol-pi]]
- [[vibe-coding-channel]] — Telegram-канал, регулярный источник наводок через инбокс ассистента; цифры передаёт точно, ограничения продукта опускает системно — спрашивать «чего в посте не сказано»
- [[claude-admin-api]] — Admin API Claude Console: пользователи, воркспейсы, ключи, сервис-аккаунты, лимиты организации; первый практический разбор (09-21) API-поверхности, до этого известной вики только фактом «вышла из беты» (08-19)

## Concepts
_(идеи, модели, принципы)_

- [[maket-kak-tz-dlya-agenta]] — почему макет, пример кода, падающий тест и рубрика работают как задание лучше прозы, и три границы приёма
- [[kalibrovka-uverennosti-resheniy]] — уверенность модели как измеримая величина (ECE), а не интуиция: чем калибровку чинят и почему ветвить конвейер по некалиброванной уверенности нельзя
...
- [[ai-security-by-design]] — как ИИ изменил кибербезопасность и принципы защиты (безопасность самого агента); 09-17: **Plugin4Shell** — четвёртый фикс подряд на узле контейнмента плагинов/маркетплейса, впервые находка внешняя (AIR Security) и задевает Claude Code/Codex/Copilot/Gemini CLI разом, а не только Claude Code; 09-14: три угрозы авторизации MCP (tool poisoning, token passthrough, confused deputy) — доверие к чужой аутентификации, отдельный класс от доверия к чужому содержимому (ContextCrush)
...
- [[novosti-2026-09-22]] — xAI выпустила Grok 4.7 (2,1 трлн параметров, та же цена, сразу в Cursor/Copilot/xAI API); OpenAI отключает Sora API 24.09 без преемника; российский закон №243-ФЗ (в силе с 01.09.2026) взят вне окна, в вики отсутствовал — но обязательная маркировка из него отложена до 01.03.2027, а не действует уже сейчас, вопреки пересказам
...
- [[laya-jev-lab-measurements]] — единственный найденный независимый замер моделей типизированных решений: локальная против облачной (57% и 7,6 мс против 78% и 588 мс), каскад, чеклист пригодности задачи, воспроизведённый отказ «уверенно и неправильно» и два отозванных автором вывода
- [[jev-ecosystem-projects]] — каталог применений модели решений: 20 проектов, из них четыре — обвязка для агентов-кодеров; отсюда «сжатие это удаление, а не пересказ», замеренный порог сита с двумя неотключаемыми предохранителями и чужая формулировка нашего правила — «блокировать может только факт»
- [[google-ax-agent-runtime]] — агент как отдельный род нагрузки: четыре примитива Google (Task, Workspace, Gateway, Model), suspend/resume; нужен кластер Kubernetes, поэтому берём идеи, а не инструмент; заодно случай сгенерированного слоя публикаций вокруг релиза
- [[habr-deepseek-harness-plugins]] — эксплуатация dsh: профили как стопка слоёв, четыре режима запуска, песочница fail-closed; из пяти проверенных утверждений статьи два искажены, одно в опасную сторону; отсюда же отрицательный урок инструмента `ralph`
- [[m3e-canvas-prompt-generator]] — инструмент, производящий референс вместо ТЗ прозой; отсюда первый в вики разбор дизайнерской ветки и правило проверять словарь макета на совместимость со стеком проекта
- [[cloudflare-quick-tunnel]] — разбор официальной документации ради того, чего нет в популярной однострочной подсказке: четыре задачи, где быстрый туннель уместен (демонстрация, вебхуки, проверка с телефона, доступ агента), и пять ограничений, из-за которых он не заменяет хостинг
- [[jev-skill-suggestion-mod]] — перечень скиллов сам стоит токенов каждую сессию (−5 496 на набор из 40); как спрятать листинг штатным `user-invocable-only`, чем платит вариант с внешним классификатором (туда уходит текст каждого запроса) и почему удалять такой мод можно только через его `setup restore`
- [[laya-local-decision-engine]] — необычно честный README: раздел ограничений подробнее раздела достижений; отсюда пропущенная ступень 0.5 в лестнице делегирования и концепт калибровки
- [[nvidia-pair-personal-ai-router]] — локальный вывод как сетевой ресурс: один Ollama-совместимый адрес на все машины дома, выбор узла по загрузке GPU и наличию модели; разбор по техблогу NVIDIA снимает главное заблуждение пересказов — запрос целиком идёт на один узел, VRAM машин не объединяется
- [[nvidia-sol-pi-harness]] — NVlabs измерила, сколько токенов агент тратит впустую: четыре механизма обвязки срезают 45–49% трафика при том же качестве (EdgeBench, GPT-5.6 Sol и Opus 5), $8,75–13,50 экономии в час против Codex и Claude Code; первый разбор, где механизмы обвязки не спроектированы, а найдены автоматическим перебором; разобран по arXiv и репозиторию, в обоих постах-наводках опущено главное ограничение — работает только поверх [[pi-coding-agent]]
...
### Все источники
- [[openjev-verdict-2-repo-review]] — репозиторий openJev-verdict-2.0 разобран по первоисточнику: заявленное превосходство над [[laya]] и [[jev-typesafe]] авторское, не независимое; установлено происхождение цифры «151M» из записи бэклога (модель-предшественник на Hugging Face)
- [[claude-cookbook-admin-api]] — официальный notebook Claude Cookbooks (`misc/admin_api.ipynb`, добавлен 18.09.2026): `client.beta.organization.*` — приглашения и роли пользователей, воркспейсы, аудит и (де)активация API-ключей, провижининг сервис-аккаунтов, лимиты организации/воркспейса; первый практический пример API-поверхности, известной вики только фактом с 08-19; не топ-находка — рассчитан на многопользовательскую организацию, у пользователя вики Console соло
- [[plugin4shell-vulnerability]] — уязвимость раскрыта независимой лабораторией AIR Security (17.09.2026): обход SHA-пиннинга плагинов в Claude Code, OpenAI Codex, GitHub Copilot и Gemini CLI; первоисточник исследователей недоступен из этого окружения, но официальный `CHANGELOG.md` Claude Code подтвердил связанный по теме фикс в версии 2.1.277 — четвёртая подряд находка на узле контейнмента плагинов/маркетплейса, впервые не из собственного changelog Anthropic
